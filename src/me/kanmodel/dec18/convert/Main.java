...
//					count++;
		...
		for (String line : res) {
			if (line.length() > 0) {
				...
				if (line.charAt(0) == '$' && line.charAt(1) == 'P') {
					if(!CodeTable.isTwo){
						System.out.print("$P" + line.substring(2, 4) + "" + Integer.toHexString(codeH).toUpperCase()
								+ Integer.toHexString(codeL).toUpperCase() + " ");
...
//						refers.add(code);
						System.out.print("$P" + line.substring(2, 4) + "" + adr + " ");
						binaryToDecimal(code);
						System.out.print(" " + line.substring(4, 6));
						CodeTable.isTwo = false;
						if(CodeTable.vars.containsKey(code)){
							System.out.print(" var:" + CodeTable.vars.get(code));
						}
					}
